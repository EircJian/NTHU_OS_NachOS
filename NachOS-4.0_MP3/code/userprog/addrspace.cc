...
AddrSpace::~AddrSpace()
{
    ...
    for(int i = 0; i < numPages; i++){
        kernel->usedFrameTable[pageTable[i].physicalPage] = 0;
    }
    delete pageTable;
}
...
// how big is address space?
    size = noffH.code.size + noffH.initData.size + noffH.uninitData.size 
			+ UserStackSize;	// we need to increase the size
						// to leave room for the stack
#endif
    // 這邊計算要被執行的 executable 總共會需要多少 pages
    numPages = divRoundUp(size, PageSize); // 這一行的size是實際上需要多少bytes
    size = numPages * PageSize; // 這一行的的size則是總共被分到多少bytes
...
// (等同於把 page offset append 到 frame number 後面)
	...
	DEBUG(dbgAddr, noffH.code.virtualAddr << ", " << noffH.code.size);
        ...
        for(total = noffH.code.size, curOffset = 0; total > 0; total -= PageSize, curOffset += PageSize){
            ...
            err = Translate(noffH.code.virtualAddr + curOffset, &physAddr, 0);
            if(err != NoException)
                RaiseException(err);
	...
	DEBUG(dbgAddr, noffH.initData.virtualAddr << ", " << noffH.initData.size);
        ...
        int total, curOffset = 0;
        for(total = noffH.initData.size, curOffset = 0; total > 0; total -= PageSize, curOffset += PageSize){
            ...
            err = Translate(noffH.initData.virtualAddr, &physAddr, 0);
            if(err != NoException)
                RaiseException(err);
