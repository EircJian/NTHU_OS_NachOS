...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            execfilePriority[execfileNum] = -1; // MP3 implementation 2-2
			cout << execfile[execfileNum] << "\n";
		} else if (strcmp(argv[i], "-ep") == 0) { // MP3 implementation 2-2
            priorityFlag = TRUE;
        	execfile[++execfileNum]= argv[++i];
            execfilePriority[execfileNum] = atoi(argv[++i]);
			//cout << execfile[execfileNum] << " with priority: " << execfilePriority[execfileNum] << "\n";
...
#endif // FILESYS_STUB
    //postOfficeIn = new PostOfficeInput(10); MP3
    //postOfficeOut = new PostOfficeOutput(reliability); MP3
...
Kernel::~Kernel()
{
    ...
    delete synchDisk;
    delete fileSystem;
    //delete postOfficeIn; MP3
    //delete postOfficeOut; MP3
    //delete usedFrameTable; // for MP2
...
void Kernel::ExecAll()
{
	...
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(i, execfile[i]); // a為目前thread的數量-1, 有用處嗎??
	}
...
int Kernel::FindAvailableFrame(){
    ...
    return -1; // 先 return -1 看看, 照理來說外面應該會有防護機制??
}