...
void* ConsumerController::process(void* arg) {
	...
	while(1){
		...
		//std::cout << "in ConsumerController::process: " << i << ", worker_queue size: " << controller->worker_queue->get_size() << '\n';
		if(controller->worker_queue->get_size() < controller->low_threshold && controller->consumers.size() > 1){
			//if(controller->consumers.size() == 1) 
			//	continue;
			Consumer *toPop = controller->consumers[controller->consumers.size()-1];
			toPop->cancel();
			//auto it = controller->consumers.rbegin();
		...
		//if(controller->worker_queue->get_size() <= 0)
		//	break;
	}
	return nullptr;
}
